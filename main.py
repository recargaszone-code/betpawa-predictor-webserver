...
def iniciar_scraper():
    ...
    while True:
        ...
        try:
            ...
            # caso contrário, confiar no Selenium Manager (selenium>=4.11) para baixar/gerenciar o driver em runtime
            service = None
            if os.path.exists("/usr/bin/chromedriver"):
                print("Usando /usr/bin/chromedriver")
                service = Service("/usr/bin/chromedriver")
            else:
                print("Nenhum chromedriver encontrado no sistema; confiando no Selenium Manager (runtime download).")

            # criar driver: passe service somente se tivermos chromedriver no filesystem;
            # caso contrário, chamar webdriver.Chrome(options=...) e deixar Selenium Manager providenciar o executável.
            if service:
